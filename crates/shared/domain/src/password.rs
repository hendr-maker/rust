...
impl Password {
    ...
    ///
    /// # Returns
    /// * `DomainResult<Password>` - Hashed password value object
    ///
    /// # Errors
    ...
    fn verify_hash(plain_text: &str, hash: &str) -> DomainResult<bool> {
        let parsed = PasswordHash::new(hash)
            .map_err(|e| DomainError::password(format!("Invalid hash format: {}", e)))?;
        Ok(Self::argon2()
